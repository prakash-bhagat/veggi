...
@Component({
  ...
  styleUrls: ['./final-invoice.component.scss']
})
export class FinalInvoiceComponent implements OnInit {

  orderId;
  chk;dataloop;sum;prod=[];
  id;date;name;society_name;address;mobile

  constructor(private roo:ActivatedRoute,private userService:UserService) { }

  ngOnInit(): void {
    this.roo.queryParams.subscribe(data=>{
      this.orderId=this.roo.snapshot.paramMap.get('orderId');
      this.userService.finalInvoice(this.orderId).subscribe(data=>{
        this.chk=data;
